...
describe('MatchesController', () => {
  ...
  it('rejects unauthenticated access to protected routes', async () => {
    ...
    await request(app.getHttpServer()).post('/matches').send({}).expect(401);
  });

  it('rejects invalid payloads with POST /matches', async () => {
    const response = await requestWithAuth('post', '/matches')
  ...
  it('creates and fetches a match', async () => {
    ...
    const response = await requestWithAuth('get', `/matches/${id}`).expect(200);
  ...
  it('updates and deletes a match', async () => {
    ...
    await requestWithAuth('patch', `/matches/${id}`)
      .send({ title: 'Updated Finals' })
      .expect(200);

    await requestWithAuth('delete', `/matches/${id}`).expect(204);
  ...
  it('lists match summaries with deterministic sorting', async () => {
    ...
    await requestWithAuth('patch', `/matches/${olderId}`).send({ date: '2026-02-01', title: 'Older' }).expect(200);
    const newerId = await createMatch();
    await requestWithAuth('patch', `/matches/${newerId}`).send({ date: '2026-03-05', title: 'Newer' }).expect(200);
    ...
    const response = await requestWithAuth('get', '/matches').expect(200);
  ...
  it('filters matches by competitor, date range, and hasVideo', async () => {
    ...
    await requestWithAuth('patch', `/matches/${alphaId}`).send({
      date: '2026-01-15',
    ...
    await requestWithAuth('patch', `/matches/${betaId}`).send({
      date: '2026-02-20',
    ...
    const byCompetitor = await requestWithAuth('get', '/matches?competitor=taylor').expect(200);
    expect(byCompetitor.body.total).toBe(2);

    const byDate = await requestWithAuth('get', '/matches?dateFrom=2026-02-01&dateTo=2026-12-31').expect(200);
    expect(byDate.body.matches).toHaveLength(1);
    expect(byDate.body.matches[0].matchId).toBe(betaId);

    const byVideo = await requestWithAuth('get', '/matches?hasVideo=true').expect(200);
  ...
  it('supports pagination via limit and offset', async () => {
    ...
    await requestWithAuth('patch', `/matches/${firstId}`).send({ date: '2026-01-01', title: 'First' }).expect(200);
    const secondId = await createMatch();
    await requestWithAuth('patch', `/matches/${secondId}`).send({ date: '2026-01-02', title: 'Second' }).expect(200);
    const thirdId = await createMatch();
    await requestWithAuth('patch', `/matches/${thirdId}`).send({ date: '2026-01-03', title: 'Third' }).expect(200);

    const response = await requestWithAuth('get', '/matches?limit=1&offset=1').expect(200);
  ...
  it('rejects unknown and invalid list query parameters', async () => {
    const response = await requestWithAuth('get', '/matches?hasVideo=maybe&limit=0&foo=bar&dateFrom=2026-13-01')
      .expect(400);
  ...
  it('creates an event with POST /matches/:id/events', async () => {
    ...
    const response = await requestWithAuth('post', `/matches/${matchId}/events`)
  ...
  it('gets events sorted by timestamp with GET /matches/:id/events', async () => {
    ...
    const response = await requestWithAuth('get', `/matches/${matchId}/events`).expect(200);
  ...
  it('updates an event with PATCH /events/:id', async () => {
    const matchId = await createMatch();
    const createResponse = await requestWithAuth('post', `/matches/${matchId}/events`)
      .send({ timestamp: 18, eventType: 'takedown_completed', competitor: 'A' })
      .expect(201);

    const response = await requestWithAuth('patch', `/events/${createResponse.body.id}`)
  ...
  it('deletes an event with DELETE /events/:id', async () => {
    ...
    await requestWithAuth('delete', `/events/${createResponse.body.id}`).expect(204);

    const listResponse = await requestWithAuth('get', `/matches/${matchId}/events`).expect(200);
  ...
  it('returns 404 behavior for missing match or event resources', async () => {
    await requestWithAuth('get', `/matches/${missingMatchId}/events`)
      .expect(404)
      .expect(({ body }: { body: { message: string } }) => {
        expect(body.message).toBe(`Match with id ${missingMatchId} was not found.`);
      });

    await requestWithAuth('post', `/matches/${missingMatchId}/events`)
      .send({ timestamp: 12, eventType: 'takedown_attempt', competitor: 'A' })
      .expect(404)
      .expect(({ body }: { body: { message: string } }) => {
        expect(body.message).toBe(`Match with id ${missingMatchId} was not found.`);
    ...
    await requestWithAuth('patch', `/events/${missingEventId}`)
    ...
    await requestWithAuth('delete', `/events/${missingEventId}`)
      .expect(404)
      .expect(({ body }: { body: { message: string } }) => {
        expect(body.message).toBe(`Timeline event with id ${missingEventId} was not found.`);
      });
  ...
  it('rejects invalid route ids before hitting services', async () => {
    const response = await requestWithAuth('get', '/matches/not-a-uuid').expect(400);
    expect(response.body.message).toContain('Validation failed (uuid is expected)');

    await requestWithAuth('patch', '/events/not-a-uuid').send({ eventType: 'sweep' }).expect(400);
    await requestWithAuth('patch', '/positions/not-a-uuid').send({ position: 'mount' }).expect(400);
    await requestWithAuth('patch', '/video/not-a-uuid').send({ title: 'Angle B' }).expect(400);
    await requestWithAuth('get', '/matches/not-a-uuid/events').expect(400);
    await requestWithAuth('get', '/matches/not-a-uuid/positions').expect(400);
    await requestWithAuth('get', '/matches/not-a-uuid/video').expect(400);
  });

  it('creates a position with POST /matches/:id/positions', async () => {
    const matchId = await createMatch();

    const response = await requestWithAuth('post', `/matches/${matchId}/positions`)
      .send({
        position: 'closed_guard',
        competitorTop: 'A',
        timestampStart: 12,
  ...
  it('gets positions sorted by timestampStart with GET /matches/:id/positions', async () => {
    ...
    const response = await requestWithAuth('get', `/matches/${matchId}/positions`).expect(200);
  ...
  it('updates a position with PATCH /positions/:id', async () => {
    const matchId = await createMatch();
    const createResponse = await requestWithAuth('post', `/matches/${matchId}/positions`)
      .send({ position: 'closed_guard', competitorTop: 'A', timestampStart: 12, timestampEnd: 20 })
      .expect(201);

    const response = await requestWithAuth('patch', `/positions/${createResponse.body.id}`)
  ...
  it('deletes a position with DELETE /positions/:id', async () => {
    ...
    await requestWithAuth('delete', `/positions/${createResponse.body.id}`).expect(204);

    const listResponse = await requestWithAuth('get', `/matches/${matchId}/positions`).expect(200);
    expect(listResponse.body).toEqual([]);
  });
  ...
  it('removes positions when parent match is deleted', async () => {
    ...
    await requestWithAuth('delete', `/matches/${matchId}`).expect(204);
    await requestWithAuth('get', `/matches/${matchId}/positions`).expect(404);
  });
  ...
  it('returns 404 behavior for missing position resources', async () => {
    await requestWithAuth('get', `/matches/${missingMatchId}/positions`)
      .expect(404)
      .expect(({ body }: { body: { message: string } }) => {
        expect(body.message).toBe(`Match with id ${missingMatchId} was not found.`);
      });

    await requestWithAuth('post', `/matches/${missingMatchId}/positions`)
      .send({ position: 'closed_guard', competitorTop: 'A', timestampStart: 12, timestampEnd: 20 })
      .expect(404)
      .expect(({ body }: { body: { message: string } }) => {
        expect(body.message).toBe(`Match with id ${missingMatchId} was not found.`);
    ...
    await requestWithAuth('patch', `/positions/${missingPositionId}`)
      .send({ position: 'mount' })
      .expect(404)
      .expect(({ body }: { body: { message: string } }) => {
    ...
    await requestWithAuth('delete', `/positions/${missingPositionId}`)
      .expect(404)
      .expect(({ body }: { body: { message: string } }) => {
        expect(body.message).toBe(`Position state with id ${missingPositionId} was not found.`);
      });
  ...
  it('creates and gets a match video', async () => {
    ...
    const createResponse = await requestWithAuth('post', `/matches/${matchId}/video`)
      .send({
        title: 'Finals angle',
        sourceType: 'remote_url',
    ...
    const getResponse = await requestWithAuth('get', `/matches/${matchId}/video`).expect(200);
  ...
  it('updates and deletes a match video', async () => {
    ...
    const updateResponse = await requestWithAuth('patch', `/video/${createResponse.body.id}`)
    ...
    await requestWithAuth('delete', `/video/${createResponse.body.id}`).expect(204);
    await requestWithAuth('patch', `/video/${createResponse.body.id}`).send({ title: 'Nope' }).expect(404);
  });

  it('returns 404 for unknown match and video IDs for video endpoints', async () => {
    await requestWithAuth('post', `/matches/${missingMatchId}/video`)
      .send({ title: 'Angle', sourceType: 'remote_url', sourceUrl: 'https://cdn.example.com/a.mp4' })
      .expect(404);

    await requestWithAuth('get', `/matches/${missingMatchId}/video`).expect(404);

    await requestWithAuth('patch', `/video/44444444-4444-4444-8444-444444444444`)
      .send({ title: 'Angle B' })
      .expect(404);
  });
  ...
  it('rejects invalid match video payloads', async () => {
    ...
    const response = await requestWithAuth('post', `/matches/${matchId}/video`)
  ...
  it('removes attached video when parent match is deleted', async () => {
    ...
    await requestWithAuth('delete', `/matches/${matchId}`).expect(204);
    await requestWithAuth('get', `/matches/${matchId}/video`).expect(404);
  });
  ...
  it('returns analytics for a valid match', async () => {
    ...
    const response = await requestWithAuth('get', `/matches/${matchId}/analytics`).expect(200);
  ...
  it('recomputes analytics totals after event and position updates and deletions', async () => {
    ...
    const createdEvent = await requestWithAuth('post', `/matches/${matchId}/events`)
      .send({ timestamp: 5, eventType: 'takedown_attempt', competitor: 'A' })
      .expect(201);

    const createdPosition = await requestWithAuth('post', `/matches/${matchId}/positions`)
      .send({ position: 'standing', competitorTop: 'A', timestampStart: 0, timestampEnd: 10 })
      .expect(201);

    const baselineAnalytics = await requestWithAuth('get', `/matches/${matchId}/analytics`).expect(200);
    ...
    await requestWithAuth('patch', `/events/${createdEvent.body.id}`)
      .send({ eventType: 'guard_pass' })
      .expect(200);

    await requestWithAuth('patch', `/positions/${createdPosition.body.id}`)
      .send({ position: 'mount', timestampStart: 2, timestampEnd: 14 })
      .expect(200);

    const updatedAnalytics = await requestWithAuth('get', `/matches/${matchId}/analytics`).expect(200);
    ...
    await requestWithAuth('delete', `/events/${createdEvent.body.id}`).expect(204);
    await requestWithAuth('delete', `/positions/${createdPosition.body.id}`).expect(204);

    const afterDeletionAnalytics = await requestWithAuth('get', `/matches/${matchId}/analytics`).expect(200);
  ...
  it('returns structured match dataset export', async () => {
    ...
    const response = await requestWithAuth('get', `/matches/${matchId}/export`).expect(200);
  ...
  it('returns 404 for export of an unknown match', async () => {
    await requestWithAuth('get', `/matches/${missingMatchId}/export`)
      .expect(404)
      .expect(({ body }: { body: { message: string } }) => {
        expect(body.message).toBe(`Match with id ${missingMatchId} was not found.`);
      });
  ...
  it('returns a dataset validation report for a match', async () => {
    ...
    const response = await requestWithAuth('get', `/matches/${matchId}/validate`).expect(200);
  ...
  it('returns 404 for analytics of an unknown match', async () => {
    await requestWithAuth('get', `/matches/${missingMatchId}/analytics`)
      .expect(404)
      .expect(({ body }: { body: { message: string } }) => {
        expect(body.message).toBe(`Match with id ${missingMatchId} was not found.`);
      });
